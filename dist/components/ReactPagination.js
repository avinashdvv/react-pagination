      ...
      React.createElement(
        ...
        React.createElement(
          ...
          currentScreenNumber !== 1 && React.createElement(
            'button',
            { className: 'previous-btn', onClick: () => this.setState({ currentScreenNumber: this.state.currentScreenNumber - 1 }) },
            ' Previous '
          ),
          times(currentNumberOfScreens, idx => React.createElement(
            'button',
            {
              key: idx,
              onClick: () => {
          ...
          currentScreenNumber < totalScreensNumber && React.createElement(
            'button',
            { className: 'next-btn', onClick: () => this.setState({ currentScreenNumber: this.state.currentScreenNumber + 1 }) },
            ' Next '
          )
        )
      )
